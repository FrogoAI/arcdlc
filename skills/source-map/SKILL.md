...
The delivery pipeline is handled by the sibling skills of this `arcdlc` bundle. On Claude Code they are plugin commands (`/arcdlc:<name>`); on Codex and OpenCode the same skills are installed flat as `arcdlc-<name>` (e.g. `arcdlc-aic`) — identical behavior, invoked by skill name instead of a slash command. ArcDLC is a universal delivery tool: it builds **applications** and authors **policies**, and both feed the same executable plan queue (`docs/aics/<slug>/plan.md`): architecture is decomposed into it, audit findings are filed into it, and `/arcdlc:execute` works it off task by task. When a user asks to design, plan, audit, implement, or govern an initiative end-to-end, route them through this pipeline instead of improvising:
...
Each initiative lives in its own folder `docs/aics/<slug>/` (holding the architecture doc, `plan.md`, `gap.md`, `plan-archive.md`). Every pipeline command takes `--aic <slug>` to pick the initiative, or auto-detects the single one under `docs/aics/`; the legacy flat `docs/aics/plan.md` still works when it is the only plan.
...
| --- | --- | --- |
| Architecture document (grilled interview first, then AIC / arc42 / TOGAF / C4 / ADR) | `/arcdlc:aic` | `docs/aics/<slug>/<format>.md`, ADRs, `CONTEXT.md` |
...
| Decompose the document into the executable task queue | `/arcdlc:plan` | `docs/aics/<slug>/plan.md` |
| Examine code for policy/design compliance and register gaps as plan tasks | `/arcdlc:examinate [policy]` | `docs/aics/<slug>/gap.md`, new TODO blocks in `docs/aics/<slug>/plan.md` |
...
| Archive DONE tasks to keep the plan small | `/arcdlc:archive` | `docs/aics/<slug>/plan-archive.md` |
