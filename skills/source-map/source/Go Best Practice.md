...
### No Private Pass-Through Functions

Do not create private helper functions whose only purpose is to call another
function or wrap a single lifecycle call without adding real logic. A private
function is allowed only when it owns meaningful work, such as:

- validation, branching, transformation, or orchestration
- cohesive setup or teardown of resources
- reusable behavior called from multiple places
- testable logic with a clear responsibility
- reducing unavoidable complexity in a large function

Avoid patterns like:

```go
func main() {
    runWorkerProcess(addr)
}

func runWorkerProcess(addr string) {
    pkgapp.NATSMain(serviceName, addr, callback)
}
```

Prefer calling the real operation directly from the caller when the extracted
function adds no behavior:
...
Do not introduce `run`, `start`, `execute`, `process`, or similar private
wrappers unless the function contains actual logic beyond forwarding arguments
to another call.
