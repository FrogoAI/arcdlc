...
| **Data carriers hold no behavior** | A struct whose only job is to move data across a boundary (request/response DTO, message payload) exports its fields and carries no business logic. |
| **Behavior belongs on the type that owns the data** | Exported fields *plus* methods is normal, not a smell: a domain entity exports its fields because the repository scans into them, and still owns the methods that enforce its rules. See **Go Server.md** `domain/{feature}/model.go`, where `Order` exports `Status` and guards its lifecycle with `Cancel()`. |
| **Don't expose mutable state that invariants depend on** | The rule is about mutation, not visibility. If a field may only change under a rule, that change goes through a method -- `o.Cancel()` checks `StatusPending`; `o.Status = StatusCancelled` at a call site silently bypasses it. Unexport the field where the compiler must enforce this and nothing scans or serializes the struct directly. |
...
- **`testing` package**: Use stdlib. No test framework is required, and none should be added -- `testify` is acceptable only in a project that already depends on it.
...
```go
// Table-driven test (Go idiom for Build-Operate-Check), standard library only
import (
    "math"
    "testing"
)
...
func TestCalculateScore(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got := CalculateScore(tt.in)
            if math.Abs(got-tt.want) > tolerance {
                t.Errorf("CalculateScore(%v) = %v, want %v (+/-%v)", tt.in, got, tt.want, tolerance)
            }
        })
    }
}
...
- **`errgroup`** (`golang.org/x/sync/errgroup`): For parallel operations with error propagation. **Not standard library** -- `golang.org/x/sync` is a separate module dependency; reach for it only when `sync.WaitGroup` plus a collected error is genuinely insufficient.
...
Adapted from Clean Code Chapter 17. Java-specific items (J1-J3) replaced with Go equivalents. Only the smells
**not** already stated in sections 1-9 above or in **Go Best Practice.md** are listed here -- the original IDs are
kept for cross-reference, so gaps in the numbering are intentional (the omitted items live in the sections above).
