...
The normative definition lives in **`mdca.md`** — principles `P1`–`P10` (cite clauses as `P3.2`), the layering rules and the purity test for domain imports in §7, the tactical rules in §8, and the compliance checklist in §11. This document does not restate them; it describes how a Go **server** realizes MDCA. Language-level rules live in **`Modern Go.md`**.
...
This document covers server architecture only. Language-level rules live in **`Modern Go.md`**; the unchanged Go idioms (formatting, naming, `defer`, receivers) are ordinary practice and are not bundled.
