...
**Ask** lines under a level are the questions `/arcdlc:aic <slug> c4` puts to the engineer before
drawing it. They are not copied into the document. A question the engineer defers goes into an Open
questions list at the end of `c4.md`, with who answers it and by which phase.
...
- Who and what talks to the system, and what flows each way, in domain words?
...
- Label relationships with **protocol AND what is transferred**: `"REST /v1/<resource>"`, `"NATS: index.sync"`, `"MongoDB driver: read/write"`.
...
- Does each container have a reason that a package inside an existing one cannot meet?
- Which store does each container own, and what is that store's throughput limit?
- When a container a request depends on is slow, and when it is gone, what does the caller receive?
...
- Which container is risky, complex or changes often enough to earn this zoom? Leave the rest out.
...
digraph C4_Dynamic {
    graph [
        label="Dynamic: Record Upload Flow"
        labelloc=t fontsize=16 fontname="Arial"
        rankdir=LR pad=0.5
    ]
    node [shape=box style="filled,rounded" fontname="Arial" fontsize=10]
    ...
    user -> api [label="1. POST /v1/.../upload"]
    api -> mongo [label="2. Batch write records"]
    api -> nats [label="3. Publish record.updated"]
    nats -> worker [label="4. Deliver event"]
    worker -> mongo [label="5. Read records"]
    worker -> nats [label="6. Publish index.sync"]
    nats -> consumer [label="7. Deliver index"]
}
```
...
digraph C4_Deployment {
    ...
    subgraph cluster_k8s {
        ...
        api_pod [label="api-service\n[Pod: Go + Fiber]\nx2 replicas"
                 fillcolor="#438DD5" fontcolor=white]
        worker_pod [label="worker\n[Pod: Go]\nx1 replica"
                    fillcolor="#2694AB" fontcolor=white]
        consumer_pod [label="consumer\n[Pod: Go]\nx3 replicas\n+<N> MB RAM"
                      fillcolor="#438DD5" fontcolor=white]
    }
    ...
    nats -> worker_pod [label="subscribe"]
    worker_pod -> mongo [label="MongoDB driver"]
    worker_pod -> nats [label="index.sync"]
    nats -> consumer_pod [label="subscribe"]
}
```
...
C4 diagrams use the blue element family — see `Diagram Conventions.md`, section `## Color Palettes`.
...
C4 diagrams follow the shared DOT → PNG workflow and the `images/c4_<level>_<name>` naming rule — see `Diagram Conventions.md`.
